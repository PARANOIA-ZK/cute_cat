...
package com.paranoia.sourcecode.collection.map;
 ...
 * 当哈希表中的条目数超过负载因子和当前容量的乘积时，哈希表是rehashed(即重新构建内部数据结构)，因此哈希表的桶数大约是桶数的两倍。
 * <p>
 * <p>
 ...
 * <p>
 * 注意，这个实现不是同步的。
 * 如果多个线程同时访问一个散列映射，并且至少有一个线程在结构上修改了映射，则必须在外部同步。
...
public class HashMap<K, V> extends AbstractMap<K, V> implements Map<K, V>, Cloneable, Serializable {
     ...
     * <p>
     * 树型阀值就是当链表长度超过这个值时，将 Node 的数据结构修改为红黑树，以便优化查找时间，默认值为8
     */
     ...
     * <p>
     * <p>
     * 这里为啥不直接使用hashCode做散列 ： hashCode是一个int值，范围在-21亿到+21亿，总更加起来有40多亿的数字，HashMap默认的容量才16
     * 用这么大的数值范围做散列明显不现实，碰撞几率太小了
     */
     ...
     * HashMap是使用默认的负载因子(0.75)和足够容纳指定映射的初始容量创建的。
     *
    ...
    final Node<K, V>[] resize() {
        ...
        if (oldCapacity > 0) {
            if (oldCapacity >= MAXIMUM_CAPACITY) {
                ...
                newThreshold = oldThreshold << 1; // double threshold  扩容加倍
            }
     ...
     * <p>
     * 替换指定哈希表的索引处bin中的所有链接节点，除非表太小，否则将resize。
     */
    ...
    public V computeIfAbsent(K key, Function<? super K, ? extends V> mappingFunction) {
        ...
        TreeNode<K, V> t = null;
        Node<K, V> old = null;
        if (size > threshold || (tab = table) == null || (tableLength = tab.length) == 0) {
            tableLength = (tab = resize()).length;
        }
        if ((first = tab[index = (tableLength - 1) & hash]) != null) {
            if (first instanceof TreeNode)
        ...
        if (v == null) {
            ...
            tab[index] = newNode(hash, key, v, first);
